...
generate_plot3 <- function( dataset_filename = "household_power_consumption.txt" , width = 480 , height = 480 )
{
  ...
  png( "figure/plot4.png" , width = width , height = height )
  # Partition:
  par( mfrow = c( 2 , 2 ) )
  # Create plot:
  ...
  # Subplot ( 1 , 2 ):
  plot( times , data$Voltage , type = "l" , xlab = "Weekday" , ylab = "Voltage" )
  # Subplot ( 2 , 1 ):
  ...
  # Subplot ( 2 , 2 ):
  plot( times , data$Global_reactive_power , type = "l" , xlab = "Weekday" , ylab = "Global Rective Power (kilowatts)" )
  # Save file:
  dev.off( )
}
