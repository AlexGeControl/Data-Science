#
# Contents:
#   Plot2 for Assignment 1 for Exploratory Data Analysis from John Hopkins @ Coursera
# Author:
...
#   generate_plot2
# Description:
#   generate plot for time series of global active power
# Notice:
#   make sure that the dataset 'household_power_consumption.txt' is in the current working directory
generate_plot2 <- function( dataset_filename = "household_power_consumption.txt" , width = 480 , height = 480 )
{
  # read data:
  ...
  png( "figure/plot2.png" , width = width , height = height )
  # Create plot:
  times <- strptime( paste( data$Date , data$Time , sep="--") , "%d/%m/%Y--%H:%M:%S" )
  plot( times , data$Global_active_power , type = "l" , xlab = "Weekday" , ylab = "Global Active Power (kilowatts)" )
  # Save as png file:
  dev.off( )
}
